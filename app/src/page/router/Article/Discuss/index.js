...
export default class Discuss extends Component {
  constructor(props) {
    ...
    Fetch.jsonp(
      `${API_ROOT}${API_VERSION}/blog/user/sign`,
      'say',
    ).then(data => {
  ...
  componentDidMount() {
    let isUserLogin = false,
      platformUser = window.sessionStorage.getItem(this.storageKey);
    if (platformUser) {
      try {
            ...
            <section className="discuss-content">
              <a className="avatar" href="javascript:void(0)">
                <img
                  src={
                    isUserLogin
                      ? platformUser.userDetail.avatar_url
                      : 'https://o42cskze7.qnssl.com/static/images/userAvatar/avatar-1.jpg'
                  }
              ...
              <div className="discuss-content-body">
                {!isUserLogin &&
                  <div className="discuss-login-mask">
                    <p>请登录后发表评论~</p>
                    <a
                      href="https://github.com/login/oauth/authorize?client_id=2c5d30e472a317b5c328&state=GitHub"
                      target="_blank">
                ...
                <div className="discuss-content-toolbar">
                  <a
                    ...
                    className={isShowEmoji ? 'active' : ''}
                    onClick={ev => this.__emojiToggle(ev)}
                    title="插入表情">
                    <i className="icon icon-smile-face" />
                  </a>
                </div>
              </div>
              <Emoji
                show={isShowEmoji}
                onClose={() => this.__emojiOnClose()}
                onSelect={this.__emojiOnSelected.bind(this)}
              />
            </section>
            <section className="discuss-submit">
              <div
                className="verify-code animation fadeIn"
                title="重新获取验证码"
                    ...
                    <li key={i}>
                      ...
                      <div className="reply-main">
                        <header>
                          <b>{userDetail.name}</b>
                          评论于
                          <time dateTime={replyTime} title={replyTime}>
                            {replyTime}
                          </time>
                        </header>
                        <section
                          dangerouslySetInnerHTML={{
                            __html: emojiDecoding(item.content),
                          }}
                        />
                      </div>
                    </li>
