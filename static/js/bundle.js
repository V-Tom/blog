window.addEventListener('DOMContentLoaded', _ => {
   ...
   */
  (function() {
    const ids = [].slice.call(
      document.querySelectorAll('#markdown [id]:not(main)')
    );
    ...
    if (hash) {
      const id = ids.find(
        $node => $node.getAttribute('id') === hash.substr(1, hash.length)
      );
      ...
      function(entries, observer) {
        entries.forEach(entry => {
          if (
            ...
            !$target.classList.contains(observerAcive)
          ) {
